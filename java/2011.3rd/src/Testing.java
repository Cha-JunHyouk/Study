	...
	public void itemStateChanged(ItemEvent e) {
		...
		if(obj == chb[0]) {
			...
			if(chb[2].isSelected()) {//선택된 경우
				...
				elementCTotal = 0;
				txt[2].setText(elementCTotal+"원");
			}
		}//if의 끝
		total = elementATotal+elementBTotal+
				elementCTotal;
		txt[3].setText(total+"원");
	}
	int elementATotal = 0;
	int elementBTotal = 0;
	int elementCTotal = 0;
	...
	public void actionPerformed(ActionEvent arg0) {
		...
		if(obj == btns[0]) {//확인 버튼을 누른경우
			...
			if(chb[0].isSelected()) {
				...
				txt[0].setText(elementATotal+"원");//소계출력
			}
			if(chb[1].isSelected()) {
				//두번째 음료가 선택된 경우
				...
				txt[1].setText(elementBTotal+"원");//소계출력
			}
			if(chb[2].isSelected()) {
				//세번째 음료가 선택된 경우
				...
				txt[2].setText(elementCTotal+"원");//소계출력
			}
			//총액 출력
